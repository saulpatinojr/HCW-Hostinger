...
- [ ] **Delete the `tf-state` branch** — `provision.yml` no longer writes to it (ADR-0017), and it holds only the root module's dead `local_file` state. `vault-config.yml` recreates the branch as an orphan on first apply if absent, so this is safe: `git push origin --delete tf-state`.
- [ ] **Decide the fate of the Terraform root module** — `infrastructure/terraform/{main,variables,outputs,versions}.tf` is no longer applied by anything. It is still validated by `ci.yml` and `vault-provision.yml`, so it costs nothing, but it describes infrastructure the repo does not manage. Either delete it, or repurpose it when the Hostinger provider can model real VPS resources.
